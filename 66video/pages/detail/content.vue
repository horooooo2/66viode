<template>
	<view class="content_page">
		...
		<view class="content">
			<rich-text :nodes="contentData.data?.content || ''"></rich-text>
		</view>
		<view class="c_bottom">
			...
			<view class="list_popup_cont">
				...
				<uni-list class="list_wrap" :border="false">
					<uni-list-item
						class="list_item"
						:border="false"
						v-for="(item,index) in sortContList"
						:key="item.id"
					>
					<template v-slot:body>
						<view class="list_cont" @click="() => changeChapter(item.id)">
							<text class="list_text" :class="item.id == contentData.chapter_id?'active' : ''">{{ index + 1 }} {{ item.title }}</text>
						</view>
					</template>
					</uni-list-item>
				</uni-list>
...
import { apiGetImageContent,apiGetNovelContent, apiGetImageChapters, apiGetNovelChapters } from "@/common/api/content.js";
...
const getContent = async () => {
	let res = {};
	if(contentData.type === 'image'){
		res = await apiGetImageContent({id: contentData.id, chapter_id: contentData.chapter_id, loading: true});
	} else if(contentData.type === 'novel'){
		res = await apiGetNovelContent({id: contentData.id, chapter_id: contentData.chapter_id, loading: true});
	}
