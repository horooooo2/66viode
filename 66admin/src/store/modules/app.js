...
function handleSidebarStatus(opened) {
  opened ? setItem('sidebarOpened','open') : setItem('sidebarOpened','closed')
}

export default {
  namespaced: true,
  state: () => ({
    sidebarOpened: getItem('sidebarOpened') == 'open',
    tagsViewList: getItem(TAGS_VIEW) || []
  }),
  mutations: {
    triggerSidebarOpened(state) {
      state.sidebarOpened = !state.sidebarOpened;
      handleSidebarStatus(state.sidebarOpened);
    },
    setSidebarOpened(state,val) {
      state.sidebarOpened = val
      handleSidebarStatus(val);
    },
    /**
     * 添加 tags
     */
